...
def recommend(user):
    #连接数据库
    DB=pymysql.connect(host='127.0.0.1', port=3306, user='root', passwd='19880314', db='recommed', charset='utf8')
    #获得数据库游标
    c=DB.cursor()
    ...
    for i in lis:
        ...
        sql3="select anime_id from anime_style where style_id="+str(i)+";"
        c.execute(sql3)
        results=c.fetchall()  
        anime_lis=[]
